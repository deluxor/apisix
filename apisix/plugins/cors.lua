...
local schema = {
  ...
  properties = {
    ...
    default_origin = {
      description =
          "use this directive to define the default origin in case any other match fails," ..
          "it falls back to 'UNAUTHORIZED' if not defined",
      type = "string",
      pattern = origins_pattern
    },
    expose_headers = {
...
function _M.header_filter(conf, ctx)
  ...
  if allow_origins or conf.default_origin then
    local origin = allow_origins or conf.default_origin
    ctx.cors_allow_origins = origin
  else
    ctx.cors_allow_origins = "UNAUTHORIZED"
  end
