...
| default_origin             | string  | False    | nil     | Defines the default origin returned when all verifications fail. If it is not defined, an `UNAUTHORIZED` origin will be returned to the client, preventing `*` by default. Make sure to use `scheme://host:port` format. For example, `https://somedomain.com:8081`. It does not support multiple origins, if you want to have the default behavior, set the value to `*`. |
